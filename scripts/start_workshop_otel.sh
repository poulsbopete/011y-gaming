...
pkill -f "otel_gaming_fleet.py" 2>/dev/null || true
...
sleep 3
nohup "$PYTHON" /root/workshop/tools/otel_gaming_fleet.py >>/tmp/workshop-fleet-supervisor.log 2>&1 &
echo $! >/tmp/workshop-fleet.pid
echo "Alloy PID $(cat /tmp/workshop-alloy.pid), Aether Games OTLP fleet $(cat /tmp/workshop-fleet.pid)"
echo "Logs: /tmp/workshop-alloy.log /tmp/workshop-fleet-supervisor.log"
